f<- function(a)
{
  ...
  # let M = 1000, set.seed(a), where a=1,2,...,M, with fixed genotypic values z_{ip}, alpha_p and eta_p,
  # and random effects: b0,b1,e_{ij}.
  ...
  # In the simulation studies, let M = 1000, set.seed(a), where a=1,2,...,M, with fixed z_ip, random effects: b0,b1,e_ij 
  ...
  ### the process of estimaiton for xi:
  ...
  #### Estimation for effect sizes: alpha's and eta's
  ## Matrix Q: the variance of random vector v
  Q<- matrix(0, nrow= 2*(P+N), ncol= 2*(P+N))
  
  # Sigma:
  Sigma<- matrix(c(est.par[1], 0, 0, est.par[2]), nrow=2, ncol=2)
  ...
  for (i in 1:N)
  {
    Q[(2*P + i), (2*P + i)]<- est.par[3]
    Q[(2*P+N + i), (2*P+ N + i)]<- est.par[4]
  }  
  
  est.v<- Q %*%t(W) %*% V1%*%(y-S%*%est.xi)
  ...
  {
    # estimated alpha's : 
    est.alpha[p]<- est.v[2*p-1] + est.xi[3]
    # estimated eta's : 
    est.eta[p]<- est.v[2*p]+ est.xi[4]
  }
  ...
  df[1:P]<-est.alpha
  df[(P+1) : (2*P)]<- est.eta
  df[(2*P+1) : (2*P+N)]<- est.g1
  df[(2*P+N+1): (2*P+2*N)] <- est.g2
  df<- data.frame(df)
  write.csv(df, paste0(a+3000, ".csv"))
